# python
# P1_AES.py

SBOX = [
    
]

RCON = [
    [0x00, 0x00, 0x00, 0x00],
    [0x01, 0x00, 0x00, 0x00],
    [0x02, 0x00, 0x00, 0x00],
    [0x04, 0x00, 0x00, 0x00],
    [0x08, 0x00, 0x00, 0x00],
    [0x10, 0x00, 0x00, 0x00],
    [0x20, 0x00, 0x00, 0x00],
    [0x40, 0x00, 0x00, 0x00],
    [0x80, 0x00, 0x00, 0x00],
    [0x1b, 0x00, 0x00, 0x00],
    [0x36, 0x00, 0x00, 0x00]
]


def sub_word(word):
    return [SBOX[b] for b in word]


def rot_word(word):
    return word[1:] + word[:1]


def bytes2matrix(text: bytes):
    return [[text[i + 4*j] for j in range(4)] for i in range(4)]


def matrix2bytes(matrix):
    return bytes([matrix[i][j] for j in range(4) for i in range(4)])


def xtime(a: int) -> int:
    # corregido: asegurar 8 bits en ambos casos
    if a & 0x80:
        return ((a << 1) ^ 0x1B) & 0xFF
    else:
        return (a << 1) & 0xFF


def mix_single_column(a: list):
    # a: lista de 4 bytes
    t = a[0] ^ a[1] ^ a[2] ^ a[3]
    u = a[0]
    a[0] = (a[0] ^ t ^ xtime(a[0] ^ a[1])) & 0xFF
    a[1] = (a[1] ^ t ^ xtime(a[1] ^ a[2])) & 0xFF
    a[2] = (a[2] ^ t ^ xtime(a[2] ^ a[3])) & 0xFF
    a[3] = (a[3] ^ t ^ xtime(a[3] ^ u)) & 0xFF


def mix_columns(state: list):
    for c in range(4):
        col = [state[0][c], state[1][c], state[2][c], state[3][c]]
        mix_single_column(col)
        for r in range(4):
            state[r][c] = col[r]


def add_round_key(state: list, key_schedule: list, round_num: int):
    for c in range(4):
        k_word = key_schedule[round_num * 4 + c]
        for r in range(4):
            state[r][c] ^= k_word[r]


def sub_bytes(state: list):
    for r in range(4):
        for c in range(4):
            state[r][c] = SBOX[state[r][c]]


def shift_rows(state: list):
    state[1][0], state[1][1], state[1][2], state[1][3] = state[1][1], state[1][2], state[1][3], state[1][0]
    state[2][0], state[2][1], state[2][2], state[2][3] = state[2][2], state[2][3], state[2][0], state[2][1]
    state[3][0], state[3][1], state[3][2], state[3][3] = state[3][3], state[3][0], state[3][1], state[3][2]


class AES128:
    def __init__(self, key: bytes):
        if len(key) != 16:
            raise ValueError("Key must be 16 bytes for AES-128")
        self._key = key
        self.key_schedule = self._key_expansion(key)

    def _key_expansion(self, key: bytes):
        w = [[key[4*i + j] for j in range(4)] for i in range(4)]
        for i in range(4, 44):
            temp = w[i-1].copy()
            if i % 4 == 0:
                temp = [a ^ b for a, b in zip(sub_word(rot_word(temp)), RCON[i//4])]
            w.append([a ^ b for a, b in zip(w[i-4], temp)])
        return w

    def encrypt_block(self, plaintext: bytes) -> bytes:
        if len(plaintext) != 16:
            raise ValueError("Plaintext must be 16 bytes")
        state = bytes2matrix(plaintext)
        add_round_key(state, self.key_schedule, 0)
        for round_num in range(1, 10):
            sub_bytes(state)
            shift_rows(state)
            mix_columns(state)
            add_round_key(state, self.key_schedule, round_num)
        # final round
        sub_bytes(state)
        shift_rows(state)
        add_round_key(state, self.key_schedule, 10)
        return matrix2bytes(state)


if __name__ == "__main__":
    # prueba rápida
    aes = AES128(b"ClaveSecreta1234")
    pt = b"MiquelNombre123!"
    ct = aes.encrypt_block(pt)
    print("ct:", ct.hex())